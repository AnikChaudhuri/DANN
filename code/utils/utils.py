...
def plotImageDistribution(data1, data2, data3, data4, dataset_names, classes_names, show=False):
    ...
    plt.bar(unique-2*(width)+(width/2), counts1, width=width, color='#FF8F77', linewidth=0.5, label='AerialPhoto')
    plt.bar(unique-(width/2), counts2, width=width, color='#FFDF77', linewidth=0.5, label='Aerialsketch')
    plt.bar(unique+(width/2), counts3, width=width, color='#8DF475', linewidth=0.5, label='photo')
    plt.bar(unique+2*(width)-(width/2), counts4, width=width, color='#77DCFF', linewidth=0.5, label='Sketch')

    ax.set_xticks(unique)
    classes = ['aeroplane', 'baseballdiamond', 'buildings', 'freeway', 'golfcourse', 'harbor', 'intersection', 
                 'mobilehomepark','overpass','parkinglot','river','runway','storagetanks','tenniscourt']
